...
unsigned load_game (char* board, FILE* data, unsigned arrayrow, 
    unsigned arraycolumn, unsigned row, unsigned column, unsigned playerCount); 
...
void play_game(unsigned height, unsigned width, unsigned playerCount, 
    ...
    unsigned rows, columns, numCounter, mRow, mColumn, pCounter, winningCount = 0;
    char* board; 
    char* input;
    char* winnerList;
    ...
    if (load != NULL) {
	pCounter = load_game (board, load, rows, columns, height, width, playerCount);
	fclose(load);
    } else {
	pCounter = 0;
    }
	...
	replayState = make_move(board, columns, mRow, mColumn, orientation, player+pCounter);
	if(replayState == 0) {
	    continue;
...
int make_move(char* board, unsigned columns, unsigned mRow, unsigned mColumn, 
	...
	if (board[place] != ' ') {
	    ...
	    board[place] = '-';
	    if((columns > place)&&check_complete(board, columns, place+columns)) {
		...
		board[place-columns] = player;
		return 2;
	    } else if((columns<place)&&check_complete(board, columns, place-columns)) {
                board[place-columns] = player;
                return 2;
...
*/
int check_complete (char* board, unsigned column, unsigned place) {
	if((board[place+1]=='|')&&(board[place-1]=='|')&&(board[place-column]=='-')&&(board[place+column]=='-')) {
	    return 1;
	}
	return 0;
}
...
**		 return 1 if the board has been filled up completely (no ' ' char).
**		 return 0 otherwise.
**
**
**
...
unsigned load_game (char* board, FILE* data, unsigned arrayrow, 
	...
	for (i = 0; i < arrayrow; i++) {
		if (!(i % 2)) {
			...
			k=0;
			for (j = 1+(i*arraycolumn); j < (i*arraycolumn)+(column*2); j+=2) {
				if (line[k] == '0') {
					k++;
					continue;
				}
			...
			k=0;
			for (j = (i*arraycolumn); j < (i*arraycolumn)+(arraycolumn); j+=2) {
                if (line[k] == '0') {
                    k++;
                    continue;
                }
	...
	for (i=0; i < arrayrow*arraycolumn; i+=2) {
		if ((i/arraycolumn)%2 == 0) {
			...
			else if ((board[i+1] == ' ') && (check_complete(board, arraycolumn, (i+1)))) {
				fprintf(stderr, "Error reading grid contents\n");
                exit(5);
			}
			else if ((board[i+1] != ' ') && (!(check_complete(board, arraycolumn, (i+1))))) {
...
/*save_game (FILE* save, char* board, unsigned pCounter, unsigned height, unsigned width)
...
**
*/
void save_game (FILE* save, char* board, unsigned pCounter, unsigned height, unsigned width) {
	char* boardState;
	char* playerState;
	char playerOccupy[4];
	int squarePosition, i, j, k;
	boardState = (char*)malloc((((width+1)*2+1)*height+width+2)*sizeof(char));
	playerState=(char*)malloc(((width+1)*height)*sizeof(char));
	k = 0;
	for (i=0; i < (height*2+1); i++) {
		if (!(i % 2)) {
			for (j=i*(width*2+2)+1; j < (i+1)*(width*2+2); j+=2) {
				if (board[j] == ' ') {
					boardState[k]='0';
					k++;
				}
			...
			for (j=i*(width*2+2); j <= (i+1)*(width*2+2); j+=2) {
                if (board[j] == ' ') {
                    boardState[k]='0';
                    k++;
                }
	...
	k++;
	for (squarePosition = (width*2+5); squarePosition < slen(board); squarePosition+=2) {
